...
def train(args):
    ...
    train_data, eval_data = blending_datasets(
        ...
        strategy,
        args.seed,
        max_count=1e8, # args.max_samples,
    ...
    train_data = train_data.select(range(args.starting_point, args.starting_point+min(args.max_samples, len(train_data))))

    # length_ids = [
    #     i for i, example in enumerate(train_data)
    #     if tokenizer.apply_chat_template(example["prompt"], tokenize=True, return_tensors="pt")[0].shape[-1] + 256 <= args.max_len
    # ]
    # train_data = train_data.select(length_ids)

    eval_data = eval_data.select(range(min(int(args.max_samples * 0.01), len(eval_data))))
...
if __name__ == "__main__":
    ...
    # Data Split for Multi Checkpointing
    parser.add_argument("--starting_point", type=int, default=0)
