...
class RewardDataset(Dataset):
    ...
    def __init__(
        ...
        if dist.is_initialized():
            cache_dir = os.environ.get("HF_HOME", os.path.expanduser("~/.cache/huggingface")).replace(".cache", "cache")
            #cache_dir = os.path.expanduser("~/.cache/huggingface").replace(".cache", "cache")
            cache_dir = os.path.join(cache_dir, "datasets/.rlhf_cache")
            # get local rank
            rank = dist.get_rank()
            print("GETING RANK", rank)
            # world size = 8 right now
            print("WARNING: ASSUMPING 8 GPUs per node")
            if rank % 8 == 0:
                processed_dataset = dataset.map(
                    self.process_data, remove_columns=dataset.column_names, num_proc=num_processors, cache_file_name=cache_dir, load_from_cache_file=False
                )
                ...
                dist.barrier()
                processed_dataset = dataset.map(
                    self.process_data, remove_columns=dataset.column_names, num_proc=num_processors, cache_file_name=cache_dir, load_from_cache_file=True
                )
            print("RANK", rank, "LOADED DATASET", len(processed_dataset))
            # clean the cache to prevent data leakage
            dist.barrier()
            if rank % 8 == 0:
                os.system(f"rm -rf {cache_dir.replace('.rlhf_cache', '')}")
        else:
            processed_dataset = dataset.map(
